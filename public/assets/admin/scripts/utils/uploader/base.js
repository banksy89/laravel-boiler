	...
	return base = {
		...
		add_image: function( container, multiple, count, name ) {
			...
			var upload_container = '<div class="js-uploader__upload_container_' + count + ' hide"></div>',
				upload_button = '<input id="js-input__' + name + '" accept="image/*" type="file" name="image[' + name + '][]" class="js-uploader__image_upload hide js-input__' + name + '" data-multiple="' + multiple + '" value="Upload Image" ' + ( multiple == 'false' ? '' : 'multiple="multiple"' ) + ' />' + ( multiple == 'false' ? '' : '<input type="hidden" name="multi[' + name + ']" value="true" />' );
		...
		get_new_croppable_image: function( image, name, crop_dimensions, file_name, file_size, count ) {
			...
			template.append( inner )
					.append( '<button type="button" class="js-uploader__delete_image_uploaded btn btn--delete" data-name="' + file_name + '" data-size="' + file_size + '">Delete</button>' )
